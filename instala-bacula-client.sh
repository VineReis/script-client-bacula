...
wget 'https://sourceforge.net/projects/bacula/files/bacula/9.4.4/bacula-9.4.4.tar.gz' -O /home/suporte/bacula/bacula-9.4.4.tar.gz

# DESCOMPACTANDO O BACULA BAIXADO NA PASTA /USR/SRC/

tar -xvzf /home/suporte/bacula/bacula-9.4.4.tar.gz -C /usr/src/
...
cd /usr/src/bacula-9.4.4/ && ./configure --enable-client-only --enable-build-dird=no --enable-build-stored=no --bindir=/usr/bin --sbindir=/usr/sbin --with-scriptdir=/etc/bacula/scripts --with-working-dir=/var/lib/bacula --with-logdir=/var/log --enable-smartalloc

# E VAMOS PARA A SEGUNDA COMPILAÇÃO

cd /usr/src/bacula-9.4.4/ && make -j8 && make install && make install-autostart-fd
